...
struct M {
    ...
    M(const M &st, const M &dr)
        : val_st(st.val_st),
          ...
          len(st.len + dr.len),
          st(min(st.st, dr.st)),
          nr_benzi(st.nr_benzi + dr.nr_benzi - (st.val_dr == dr.val_st ? 1 : 0)) {
        if (st.nr_st == st.len && st.val_st == dr.val_st) nr_st += dr.nr_st;
        if (dr.nr_dr == dr.len && dr.val_dr == st.val_dr) nr_dr += st.nr_dr;
        if (st.val_dr == dr.val_st) optim = max(optim, st.nr_dr + dr.nr_st);
    }
...
struct K {
    ...
    M operator*(const M &m) const {
        ...
        M t =
            (st / ki->len == dr / ki->len)
                ? ki->rmq[__builtin_ctz(dr - st)][st % ki->len]
...
pair<int, int> solve(int l, int r) {
    auto t = query(l, r);
    return make_pair(t.nr_benzi - (t.val_st == t.val_dr ? 1 : 0), t.optim == t.len ? t.len : (t + t).optim);
}
